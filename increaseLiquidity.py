...
def increaseLiquidity(network, tokenId, \
    ...
    try:
        ...
        if "Unable to complete request at this time" in str(e) or 'asyncio.exceptions.CancelledError' in str(e):
            #N.B. wait!
            logger.error(network + ', wait_for_transaction_receipt() in increaseLiquidity() failed with error ' + traceback.format_exc(limit=0))
            tx_receipt = delay_error(network, init, tx_hash)
        else:
            logger.error(network + ', wait_for_transaction_receipt() in increaseLiquidity() failed after TIMEOUT_SEC = ' + str(TIMEOUT_SEC) + ' with error ' + traceback.format_exc(limit=0) + '. Try again with higher gas!')
            return False
    ...
    return liquidity, amount0, amount1, blockNumber, nonce
