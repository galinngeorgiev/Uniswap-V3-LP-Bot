...
def collect(tokenId, network='mumbai', max_fee_per_gas=MAX_FEE_PER_GAS, max_priority_fee_per_gas=MAX_PRIORITY_FEE_PER_GAS, address=ADDRESS_TESTNET, private_key=PRIVATE_KEY, init=None, nonce=None):
    ...
    try:
        ...
        logger.error(network + ', send_raw_transaction() in collect() failed . Try again with higher gas! ' + traceback.format_exc(limit=0))
        return False
    ...
    try:
        ...
        if "Unable to complete request at this time" in str(e) or 'asyncio.exceptions.CancelledError' in str(e) or 'already known' in str(e):
            #N.B. wait!
