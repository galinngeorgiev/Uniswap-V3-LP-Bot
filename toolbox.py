...
class Initialization():
	def __init__(self, network='mumbai'):
		...
		if network == 'ethereum':
			...
			self.network_coin_symbol, self.network_coin_decimal = COIN_SYMBOL_MAINNET, COIN_DECIMAL_MAINNET
		...
		#	self.pool_fee = POOL_FEE_KOVAN
			...
			self.network_coin_symbol, self.network_coin_decimal = COIN_SYMBOL_POLYGON, COIN_DECIMAL_POLYGON
		elif network == 'mumbai':
			if NODE_INFURA:
...
def current_LP_positions(network='mumbai',address=ADDRESS_TESTNET, startBlockNumber=0, flag_token_alchemy=TOKEN_ALCHEMY, init=None):
	...
	if flag_token_alchemy:
		...
		logger.info(network + ', ...scan API NFTs in the account = ' + str(tokenIds) + ', in current_LP_positions()')
		logger.info(network + ', ...scan API burned NFTs in the account = ' + str(tokenIds_burned) + ', in current_LP_positions()')
...
def current_pool_price(network='mumbai', block_num=None, price_list=[], price_list_median=[], pool_liquidity_list=[], pool_liquidity_median=0.,\
						price_mad=0., max_price_mad=0.,	priceLower=[], priceUpper=[], swap_volume_token1=[], rel_swap_volume_token1=[],\
					   swap_flow_token1=[], flag_price_alchemy=PRICE_ALCHEMY, init=None):
	#logger.info(pool_liquidity_list)
	price_median, signed_token1_quantity = None, 0.
	headers = {"Accept": "application/json"}
	if init is None:
		init = Initialization(network)
		...
		while i < len_data - 1:
			...
			if token0_quantity is not None and token0_quantity != 0. and token1_quantity is not None and\
				...
				if price_condition:
					...
					if price_median is not None and abs(price / price_median - 1.) < MAX_PRICE_RETURN_PER / 100:
						#N.B. Can not use '+=' on a list!
						...
						if (len_data - 1 > 0 and len_data - 1 <= 100) or (len_data - 1 > 100 and i > len_data - 100):
							out_line = network + ", tx processed " + str(len(data)) \
								...
								+ ", p med=" + "{:1.5f}".format(price_list_median[-1])\
	...
	return [GMTtimeStamp, blockNumber, txIx, price_list, price_list_median, pool_liquidity_list, pool_liquidity_median, signed_token1_quantity,\
