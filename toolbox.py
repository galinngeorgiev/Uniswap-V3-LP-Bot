...
def current_pool_price(network='mumbai', block_num=None, price_list=[], price_median=0., pool_liquidity_list=[], pool_liquidity_median=0.,\
						price_mad=0., max_price_mad=0.,	swap_volume_token1=0., rel_swap_volume_token1=0., swap_flow_token1=0., init=None):
	...
	try:
		...
		while i < len_data - 1:
			...
			if token0_quantity is not None and token0_quantity != 0. and token1_quantity is not None and\
				...
				swap_volume_token1 += token1_quantity
				swap_flow_token1 += longShort * token1_quantity

				if PRICE_API_LOGS:
					rel_swap_volume_token1 += token1_quantity / pool_liquidity
				else:
					#N.B. Compute the pool price of token1 w.r.t. token0
					...
					if len(price_list) > 0 and price != price_list[-1]:
						...
						rel_swap_volume_token1 += abs(1 / np.sqrt(price) - 1 / np.sqrt(price_list[-1]) ) / 10 ** (0.5 * (init.token0_decimal + init.token1_decimal))
...
def delay_error(network, init, tx_hash):
	...
	for i in range(DELAYED_ERR_NUM_ITERATIONS):
		...
		try:
			...
			#N.B. If tx is not mined, return True!
