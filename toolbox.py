...
def current_LP_positions(network='mumbai',address=ADDRESS_TESTNET, startBlockNumber=0, init=None):
	...
	logger.info(network + ', NFTs in the account = ' + str(tokenIds) + ', in current_LP_positions()')
	if not TOKEN_ALCHEMY:
		logger.info(network + ', burned NFTs in the account = ' + str(tokenIds_burned) + ', in current_LP_positions()')
	...
	try:
		...
		while i < len_data - 1:
			...
			if token0_quantity is not None and token0_quantity != 0. and token1_quantity is not None and\
				...
				if PRICE_ALCHEMY:
					price_condition = token1_quantity * price > MIN_TOKEN1_VALUE
					if txIx > 0:
						price_condition = price_condition and int(txIx) <= MAX_TX_IX
				else:
