...
def main(network='mumbai'):
    ...
    priceLower, tickLower, priceUpper, tickUpper, priceLP = (NUM_LP + 1) * [None], (NUM_LP + 1) * [None], (NUM_LP + 1) * [None], (NUM_LP + 1) * [None], (NUM_LP + 1) * [None]
    ...
    init_condition, unwind_condition = (NUM_LP + 1) * [False], (NUM_LP + 1) * [False]
    count_LP_SWAP_attempts = 0

    #initialize session
    session_count_LP, session_count_LP_SWAP, session_count_unwind_stop, session_count_unwind_pool_liq, session_count_unwind_max_price_mad,\
    ...
    while i <= main_loop_end:
        ...
        #N.B. Try again failed mint()
        if not flag_mint:
            init_condition[j] = True
        else:
            ...
            if flag_LP_SWAP and priceLP[NUM_LP] is None and amount_to_swap_token0 != 0.:
                ...
                amount0_LP = amount_to_swap_token0 * 10**init.token0_decimal if amount_to_swap_token0 > 0 else 0.
                amount1_LP = - amount_to_swap_token0 / price * 10**init.token1_decimal if amount_to_swap_token0 < 0 else 0.
                if amount0_LP > 0 or amount1_LP > 0:
                    init_condition[NUM_LP] = True
            ...
            if tokenIds[:NUM_LP] == NUM_LP * [None] and i < int(RUNTIME_SEC / DELAY_LOOP_SEC) - NUM_LP * MAX_ATTEMPS_FAILED_TX:
                amount0_LP = NUM_TOKEN0_LP / NUM_LP * 10**init.token0_decimal
                amount1_LP = np.min([NUM_TOKEN1_LP / NUM_LP, NUM_TOKEN0_LP / NUM_LP / price]) * 10**init.token1_decimal
                ...
                if not (int(time.strftime('%w', time.gmtime())) != 6 and int(time.strftime('%w', time.gmtime())) != 0 and\
                    ...
                    init_condition[:NUM_LP] = NUM_LP * [True]
                    ...
                    if EVENT_LOGS and len(pool_liquidity_list) > 0 and\
                        ...
                        init_condition[:NUM_LP] = [init_condition[k] and True for k in range(NUM_LP)]
                    else:
                        logger.info('')
                        ...
                        init_condition[:NUM_LP] = NUM_LP * [False]
                    ...
                    if blockNumber_init is None:
                        ...
                        if  block_num >= blockNumber_init + MIN_INIT_AFTER_BLOCKS:
                            if abs(price - price_init) / price_init * 10000 >= MIN_INIT_AFTER_PRICE_RET_BP: # and (price - price_init) * price_init > 0:
                                init_condition[:NUM_LP] = [init_condition[j] and True for j in range(NUM_LP)]
                            else: 
                                #logger.info('')
                        ...
                        if init_condition[:NUM_LP] == NUM_LP * [True]:
            ...
            if not flag_failed_tx:
                j = 0
            while j <= NUM_LP:
                #if j == NUM_LP and not flag_LP_SWAP: #N.B. When j += 1, this line is hit, but if init_condition[NUM_LP] == False, nothing will happen!
                #    j = 0
                #    break
                if init_condition[j]:
                    if (flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect and flag_burn) or not flag_mint:
                        ...
                        if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                            ...
                            logger.info(network + ", session" +\
                                    ...
                                    ", swap slippage & pool fees=" + "{:1.2f}".format(session_slippage_and_pool_fee_pl_bp) + " bp") # +\
                                    #", iv) hedge RL=" + "{:1.2f}".format(session_hedge_RL_pl_bp) + " bp")
                            logger.info('network, END session')
                            return False
                #N.B. 'if not flag_failed_tx' is needed, in case delete tx-s fail 
                if not flag_failed_tx:
                    j += 1
        #######################################################################  
        if tokenIds != (NUM_LP + 1) * [None]:
            ...
            if not flag_failed_tx:
                ...
                while j < NUM_LP and i < int(RUNTIME_SEC / DELAY_LOOP_SEC) - NUM_LP * MAX_ATTEMPS_FAILED_TX:
                    ...
                    if priceLower[j] is not None and priceUpper[j] is not None and priceLP[j] is not None:
                        ...
                        #logger.info('')
                        logger.info(network + ", " + str(j) + "-th LP" +\
                                            ": dist-to-lower-bound / unwind = " + "{:1.2f}".format((1. - priceLower[j] / price_DECIMAL) * 100) + "%" +\
                                            " / " + "{:1.2f}".format(unwind_distance_to_bound[j][0] * 100) + "%" +\
                                            ", dist-to-upper-bound / unwind = " + "{:1.2f}".format((priceUpper[j] / price_DECIMAL - 1.) * 100) + "%" +\
                        ...
                        if abs(amount1_invested[j]) > 0 and price_DECIMAL > priceLP[j]:
            ...
            #N.B. Unwind failed tx-s; avoid failed mint()
            ...
            #N.B. Unwind during quiet hours
                ...
                if flag_LP_SWAP:
                    ...
                    #N.B. Need smaller price!
                    if amount_to_swap_token0 > 0:
                        #N.B. Unwinding successfully LP swap
                        if priceLower[NUM_LP] is not None and priceLower[NUM_LP] / price_DECIMAL - 1. >= LP_SWAP_DISTANCE_TO_BOUND_PER / 100:
                            logger.info('')
                            ...
                            flag_LP_SWAP = False
                            unwind_condition[NUM_LP] = True
                        #N.B. unwinding failed LP swap!
                        if priceUpper[NUM_LP] is not None and price_DECIMAL / priceUpper[NUM_LP] - 1. >= LP_SWAP_UNWIND_DISTANCE_PER / 100:
                            count_LP_SWAP_attempts += 1
                            logger.info('')
                    ...
                    if amount_to_swap_token0 < 0:
                        #N.B. Unwinding successfully LP swap
                        if priceUpper[NUM_LP] is not None and price_DECIMAL / priceUpper[NUM_LP] - 1. >= LP_SWAP_DISTANCE_TO_BOUND_PER / 100:
                            logger.info('')
                            logger.info(network + ", UNWIND successful LP swap because price = " + str(price) +\
                               " is LP_SWAP_DISTANCE_TO_BOUND_PER / 100 = " + str(LP_SWAP_DISTANCE_TO_BOUND_PER / 100) + "%" +\
                               " above priceUpper[NUM_LP] = " + str(priceUpper[NUM_LP] / 10**init.token0_decimal * 10**init.token1_decimal))
                            ...
                            unwind_condition[NUM_LP] = True
                        #N.B. unwinding failed LP swap!
                        if priceLower[NUM_LP] is not None and priceLower[NUM_LP] / price_DECIMAL - 1. >= LP_SWAP_UNWIND_DISTANCE_PER /100:
                            count_LP_SWAP_attempts += 1
                            logger.info('')
                            logger.info(network + ", UNWIND failing LP swap because price = " + str(price) +\
                               " is LP_SWAP_UNWIND_DISTANCE_PER / 100 = " + str(LP_SWAP_UNWIND_DISTANCE_PER / 100) + "%" +\
                ...
                if EVENT_LOGS and abs(signed_token1_quantity) / pool_liquidity_list[-1]  *\
                    ...
                    logger.info('')
                    logger.info(network + ", UNWIND all LP positions because abs(signed_token1_quantity) / pool_liquidity * 10000 = " +\
            ...
            while j <= NUM_LP:
                ...
                #4. Delete the above LP position
                    if (flag_mint and flag_increaseLiquidity and flag_hedgeRL and flag_collect and flag_burn) or not flag_decreaseLiquidity:
                    ...
                    #time.sleep(3)
                    if (flag_mint and flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_burn) or not flag_collect:
                        if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                    ...
                    ###################################################
                    #6. Burn the NFT correponding to tokenIds[]: otherwise the NFT stays in the wallet & can be seen running current_LP_positions()
                    ...
                    #N.B. swap() is after burn() because swap() does not depend on unwind_condition: if burn() fails, swap() is delayed or fails!
                    if (flag_mint and flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect) or not flag_burn:
                        if count_failed_tx < MAX_ATTEMPS_FAILED_TX:                        
                ...
                #N.B. 'if not flag_failed_tx' is needed, in case mint() fails 
                if not flag_failed_tx:         
                    j += 1
            ...
            if (not flag_failed_tx):
                ...
                if unwind_condition[:NUM_LP] != NUM_LP * [None] and i < int(RUNTIME_SEC / DELAY_LOOP_SEC) - NUM_LP * MAX_ATTEMPS_FAILED_TX:
                    logger.info(network + ", session" +\
                            #": i) w/o hedge RL=" + str(session_borrow_pl_token1[j]) + " token1" +\
                            ": borrow p&l w/o swaps= " + "{:1.2f}".format(sum(session_borrow_pl_bp)) + " bp"  +\
                            ", RL = " + "{:1.2f}".format(sum(session_RL_bp)) + " bp" +\
        ...
        if flag_mint and flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect and flag_burn and\
            ...
            #N.B. At the end, standard swap is executed!
            if i == main_loop_end - MAX_ATTEMPS_FAILED_TX:
                if abs(amount_to_swap_token0) > 0.:
                    swap_condition = True
            else:
                #N.B. tx batch swap
                ...
                if abs(tx_batch_amount_to_swap_token1) > MIN_TX_BATCH_SWAP_PER / 100 * sum(tx_borrow_invested_token1):
                    ...
                    logger.info(network + ", no tx batch swap: abs(tx_batch_amount_to_swap_token1) = " + str(abs(tx_batch_amount_to_swap_token1)) +\
                       " <= min tx swap threshold = " + str(MIN_TX_BATCH_SWAP_PER / 100 * sum(tx_borrow_invested_token1)))
                    ...
                    if abs(session_res_num_token1) > MIN_SESSION_SWAP_PER / 100 * sum(tx_borrow_invested_token1) or\
                        ...
                        logger.info(network + ", no session swap: abs(session_res_num_token1)=" + str(abs(session_res_num_token1)) +\
                                " <= min session swap token1=" + str(MIN_SESSION_SWAP_PER / 100 * sum(tx_borrow_invested_token1)) + ", continue with next iter...")
                        swap_condition = False
            ...
            if swap_condition and abs(amount_to_swap_token0) > SWAP_EPSILON_PER / 100  * sum(tx_borrow_invested_token1) * price :
                ...
                if LP_SWAP and count_LP_SWAP_attempts < LP_SWAP_MAX_ATTEMPTS_FAILED_TX and i < main_loop_end - MAX_ATTEMPS_FAILED_TX:
                    ...
                    if isinstance(result, tuple):
                        ...
                        logger.info(network + ", swap slippage & pool fees=" +\
        ...
        if i >= int(RUNTIME_SEC / DELAY_LOOP_SEC) and (not flag_LP_SWAP):
