...
def main(network='mumbai'):
    ...
    session_count_LP_swap, session_count_LP_swap_failed, session_count_unwind_stop_profit, session_count_unwind_stop_loss, \
            ...
            session_count_hedge_RL, session_count_non_LP_swaps, session_count_unwind_flow, session_count_tx =\
            0, 0, 0, 0, 0, 0, 0, 0, 0, (NUM_LP + 1) * [(0, 0, 0, 0)] + [(0,)]
        ...
        if (not flag_failed_tx) and (flag_change_tokenIds or i % PERIOD_CURRENT_LP_POSITIONS_ITERATIONS == 0):
            ...
            if isinstance(result, list):
                ...
                while k <= NUM_LP and l < len(result):
                    ...
                    #N.B. i.e., if mint() occured while tokenIds[np.argsort(mint_time)[k]] = None, accept mint-assigned tokenId!
                    if tokenIds[np.argsort(mint_time)[k]] is None:
                        k += 1
                        continue

                    #N.B. If tokenIds[np.argsort(mint_time)[k]] = tokenIds[NUM_LP], accept mint-assigned tokenId!
                    #N.B. Increase k by 1 if tokenIds[np.argsort(mint_time)[k]] = tokenIds[NUM_LP], or 
                    #N.B. Increase l by 1 if result[l] == tokenIds[NUM_LP]
                    if tokenIds[np.argsort(mint_time)[k]] == tokenIds[NUM_LP] or result[l] == tokenIds[NUM_LP]:
                        k += 1
                        l += 1
                        continue
                ...
                if len([t for t in tokenIds if t is not None]) > NUM_LP + 1:
                    ...
                    logger.info(network + ", session: count tx-s = " + "{:d}".format(sum([sum(s) for s in session_count_tx])) +\
                                  " = [" + str(["{:d}".format(s[0]) for s in session_count_tx[:-1]]) +\
                                  ", " + str(["{:d}".format(s[1]) for s in session_count_tx[:-1]]) +\
                                  ", " + str(["{:d}".format(s[2]) for s in session_count_tx[:-1]]) +\
                                  ", " + str(["{:d}".format(s[3]) for s in session_count_tx[:-1]]) +\
                                  ", " + "{:d}".format(session_count_tx[-1][0]) + '] ')
                    logger.info(network + ", session: gas fees on successful tx-s= " +\
                ...
                if mint_condition[j]:
                    ...
                    if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                        ...
                        if isinstance(result, tuple):
                            ...
                            session_count_tx[j] = tuple(map(sum, zip(session_count_tx[j], (1, 0, 0, 0) )))
                        ...
                        if flag_increaseLiquidity and flag_hedgeRL and flag_collect and flag_burn:
                            ...
                            if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                                ...
                                if isinstance(result, tuple):
                                    ...
                                    session_count_tx[j] = tuple(map(sum, zip(session_count_tx[j], (0, 1, 0, 0) )))
                        ...
                        if flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_burn:
                            if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                                ...
                                if isinstance(result, tuple):
                                    gas_fee, amount0, amount1, nonce = result
                                    blockNumber_init, price_init = block_num, price #if signed_token1_quantity > 0 else -price
                                    session_count_tx[j] = tuple(map(sum, zip(session_count_tx[j], (0, 0, 1, 0) )))
                        ...
                        if flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect:
                            ...
                            if count_failed_tx < MAX_ATTEMPS_FAILED_TX:                        
                                ...
                                if isinstance(result, tuple):
                                    ...
                                    session_count_tx[j] = tuple(map(sum, zip(session_count_tx[j], (0, 0, 0, 1) )))
            ...
            if flag_LP_swap and (count_LP_swap_attempts >= LP_SWAP_MAX_ATTEMPTS_FAILED_TX or i >= main_loop_end - MAX_ATTEMPS_FAILED_TX):
                ...
                if abs(session_res_num_token0) >= MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price or\
                    ...
                    logger.info(network + ", NO session swap: abs(session_res_num_token0) = " + "{:1.4f}".format(abs(session_res_num_token0)) +\
                            " <= min session swap token1 = " + str(MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price)  + ' ' + init.token1_symbol)
                    #N.B. tx batch swap
                ...
                if abs(amount_to_swap_token0) >= SWAP_EPSILON_PER / 100  * NUM_INVESTED_TOKEN1_LP * price:
                    ...
                    #conventional swap
                        ...
                        if isinstance(result, tuple):
                            ...
                            session_count_tx[-1] = tuple(map(sum, zip(session_count_tx[-1], (1, ) )))
