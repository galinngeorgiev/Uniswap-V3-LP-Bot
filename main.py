...
-                             ' >= ' + str(MAX_FAILED_TX_GAS_USED)  + ' ' + init.network_coin_symbol +  ', stop main()!')
        ...
        if flag_mint and flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect and flag_burn:
            ...
            if flag_LP_swap and (count_LP_swap_attempts >= LP_SWAP_MAX_ATTEMPTS_FAILED_TX or\
                ...
                session.res_num_token0 += amount0_LP[NUM_LP] / 10**init.token0_decimal
                session.res_num_token1 += amount1_LP[NUM_LP] / 10**init.token1_decimal
                flag_LP_swap, j = False, 0 #N.B. Affects the end of the loop
                flag_mint, mint_condition[NUM_LP] = True, False
                swap_condition = True

            #N.B. session swap first: overwrites tx batch swap because session.res_num_token0 includes tx_batch_session.amount_to_swap_token0
            if abs(session.res_num_token0) >= MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price or\
                ...
                logger.info(network + ", NO session conventional swap_condition = True: abs(session.res_num_token0) = " +\
                ...
                if abs(tx_batch_amount_to_swap_token0) >= MIN_TX_BATCH_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price:
                    ...
                    logger.info(network + ", NO tx batch conventional swap_condition = True either: abs(tx_batch_amount_to_swap_token0) = " +\
