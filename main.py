...
def main(network='mumbai'):
    ...
    count_LP_swap_attempts, count_failed_tx, count_failed_price = 0, 0, 0
    ...
    while i <= main_loop_end:
        ...
        if tokenIds[:NUM_LP] == NUM_LP * [False]:
            logger.info(network + ', flag_unwind = False because tokenIds[:NUM_LP] == NUM_LP * [False]')
            flag_unwind = False

        if sum([sum(s) for s in session_success_tx_gas_fees]) >= MAX_SUCCESS_TX_GAS_USED or\
                sum([sum(s) for s in session_failed_tx_gas_fees]) >= MAX_FAILED_TX_GAS_USED:
            logger.info('')
            logger.error(network + ', session_success_tx_gas_fees = ' +\
            ...
            logger.info(network + ", session: " + init.token1_symbol + "-ref-frame p&l w/o swaps = " +\
                ...
                ", token0 collected - invested in " + init.token1_symbol + "-ref-frame p&l = " +\
        ...
        if (not flag_failed_tx) and (flag_change_tokenIds or i % PERIOD_CURRENT_LP_POSITIONS_ITERATIONS == 0):
            ...
            if isinstance(result, list):
                ...
                if len(result) != len([tokenIds[j] for j in range(len(tokenIds)) if tokenIds[j] is not None]):
                    ...
                    logger.error(network + ', more than ' + str(NUM_LP + 1) + ' simultaneous LP positions are NOT allowed, flag_unwind is raised!')
                    flag_unwind = True
                elif (not LP_SWAP and len([t for t in tokenIds if t is not None]) > NUM_LP):
                    logger.error(network + ', more than ' + str(NUM_LP) + ' simultaneous LP positions are NOT allowed, flag_unwind is raised!')
                    flag_unwind = True
            ...
            if isinstance(result, list):
                ...
                if not flag_failed_tx and i < int(RUNTIME_SEC / DELAY_LOOP_SEC) - NUM_LP * MAX_ATTEMPS_FAILED_TX:
                    ...
                    if count_failed_price >= MAX_ATTEMPS_FAILED_PRICE:
                        EVENT_LOGS, PRICE_ALCHEMY = False, False
                        logger.info(network + ", count_failed_price = " + str(count_failed_price) + " >= MAX_ATTEMPS_FAILED_PRICE = " +\
                            str(MAX_ATTEMPS_FAILED_PRICE) + ", switch to EVENT_LOGS, PRICE_ALCHEMY = False, False")
