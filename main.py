    ...
    while i <= main_loop_end:
        ...
        if flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect and flag_burn:
            ...
            if not flag_mint:
                ...
                mint_LP_swap_condition = flag_LP_swap and priceLP[NUM_LP] is None and amount_to_swap_token0 != 0.
                ...
                if CHANGE_LP_POSITION_TO_INIT_PRICE:
                    mint_LP_swap_condition = mint_LP_swap_condition and i >= int(RUNTIME_SEC / DELAY_LOOP_SEC)
                if  mint_LP_swap_condition:
                            ...
                            if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                                ...
                                if isinstance(result, tuple):
                                    ...
                                    if j < NUM_LP:
                                        ...
                                        #N.B. Use tx_batch_amount_to_swap_token0 instead of tx_batch_amount_to_swap_token1 because token0 collected creates WETH-ref-frame p&l & price can change!
        ...
        if not flag_failed_tx:
            if count_LP_swap_attempts >= LP_SWAP_MAX_ATTEMPTS_FAILED_TX or i >= main_loop_end - MAX_ATTEMPS_FAILED_TX or \
                ...
                if flag_LP_swap and (count_LP_swap_attempts >= LP_SWAP_MAX_ATTEMPTS_FAILED_TX or i >= main_loop_end - MAX_ATTEMPS_FAILED_TX):
                    ...
                    if abs(session_res_num_token1) >= MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP or\
                        ...
                        if abs(tx_batch_amount_to_swap_token0) >= MIN_TX_BATCH_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price:
                            ...
                            logger.info(network + ", no tx batch swap either: abs(tx_batch_amount_to_swap_token0) = " + str(abs(tx_batch_amount_to_swap_token0)) +\
                               " <= min tx swap threshold = " + str(MIN_TX_BATCH_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price) + ", continue with next iter...")
                ...
                if swap_condition and abs(amount_to_swap_token0) >= SWAP_EPSILON_PER / 100  * NUM_INVESTED_TOKEN1_LP * price :
                    logger.info(network + ", swap_condition is raised and amount_to_swap_token0 = " +\
                            str(amount_to_swap_token0) + " with abs(amount_to_swap_token0) > " +\
