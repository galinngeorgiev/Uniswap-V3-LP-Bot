...
def main(network='mumbai'):
    ...
    while i <= main_loop_end:
        ...
        if (not flag_failed_tx) and (flag_change_tokenIds or i % PERIOD_CURRENT_LP_POSITIONS_ITERATIONS == 0):
            ...
            result = current_LP_positions(network, address=Address, startBlockNumber=startBlockNumber, init=init) 
            if isinstance(result, list):
            ...
            if isinstance(result, list):
                ...
                if (not flag_failed_tx) and (not flag_unwind) and i < int(RUNTIME_SEC / DELAY_LOOP_SEC) - NUM_LP * MAX_ATTEMPS_FAILED_TX:
                    ...
                    if count_failed_price >= MAX_ATTEMPS_FAILED_PRICE:
                        EVENT_LOGS, PRICE_ALCHEMY = not EVENT_LOGS, not PRICE_ALCHEMY
                        logger.info(network + ", count_failed_price = " + str(count_failed_price) + " >= MAX_ATTEMPS_FAILED_PRICE = " +\
                            str(MAX_ATTEMPS_FAILED_PRICE) + ", switch EVENT_LOGS, PRICE_ALCHEMY = not EVENT_LOGS, not PRICE_ALCHEMY")
                    if not result:
...
-                             ' >= ' + str(MAX_FAILED_TX_GAS_USED)  + ' ' + init.network_coin_symbol +  ', stop main()!')
            current_LP_positions(network, Address, startBlockNumber, init)
            ...
            while j <= NUM_LP:
                ...
                if mint_condition[j]:
                    ...
                    if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                        ...
                        #current_LP_positions(network, address=Address, session.startBlockNumber, init=init)
        ...
        if tokenIds != (NUM_LP + 1) * [None]:
            if not flag_failed_tx:
                ...
                logger.info(network + ", session.res_num_token0 = " +  "{:1.2f}".format(session.res_num_token0) + ' ' + init.token0_symbol +\
                    ...
                    if flag_LP_swap:
                        ...
                        if amount0_LP[NUM_LP] is not None and amount0_LP[NUM_LP] > 0:
                            ...
                            if priceLower[NUM_LP] is not None and 1. - priceLower[NUM_LP] / price_DECIMAL <= 0.: #- LP_SWAP_DISTANCE_TO_BOUND_PER / 100:
                                ...
                                flag_LP_swap, session.amount_to_swap_token0 = False, 0.
                                unwind_condition[NUM_LP] = True
                            ...
                            if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                                ...
                                if isinstance(result, tuple):
                                    ...
                                    if j < NUM_LP:
                                        ...
                                        #N.B. session
                                        session.res_num_token0 += tx_res_num_token0
                                        session.res_num_token1 += tx_res_num_token1

                                    elif j == NUM_LP: #N.B. flag_LP_swap = False above!
                                        #N.B. amount0 > 0, amount1 ~ 0 or vice versa!
                                        session.token0_price_collected.append(((amount0 - amount0_invested[j]) / 10**init.token0_decimal, price))
                                        session.token1_price_collected.append(((amount1 - amount1_invested[j]) / 10**init.token1_decimal, price))
        ...
        if flag_mint and flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect and flag_burn:
            ...
            if flag_LP_swap and (count_LP_swap_attempts >= LP_SWAP_MAX_ATTEMPTS_FAILED_TX or i >= main_loop_end - MAX_ATTEMPS_FAILED_TX):
                if count_LP_swap_attempts >= LP_SWAP_MAX_ATTEMPTS_FAILED_TX:
                    logger.info(network + ", LP swap tx failed " +\
                            str(count_LP_swap_attempts) + " times, conventional swap is executed with session.amount_to_swap_token0 = " + str(session.amount_to_swap_token0))
                if i == main_loop_end - MAX_ATTEMPS_FAILED_TX:
                    logger.info(network + ", LP swap tx with tokenId = " + str(tokenIds_burned[-1]) + " failed because iterations reached main_loop_end" +\
                            ", conventional swap is executed with session.amount_to_swap_token0 = " + str(session.amount_to_swap_token0)  + ' ' + init.token0_symbol)
                #N.B. Can use session.amount_to_swap_token0 despite the fact that this is not immediately after session.amount_to_swap_token0 was set up:
                #N.B. because there is only one LP swap and conventional swap & LP swap can not co-exist: session.amount_to_swap_token0 has not changed!
                ...
                #N.B. session swap: overwrites tx batch swap because session.res_num_token0 includes tx_batch_session.amount_to_swap_token0
                if abs(session.res_num_token0) >= MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price or\
                        i >= int(RUNTIME_SEC / DELAY_LOOP_SEC):
                    if session.res_num_token0 > 0:
                        session.amount_to_swap_token0 = session.res_num_token0
                    elif session.res_num_token1 > 0:
                        session.amount_to_swap_token0 = -session.res_num_token1 * price
                    
                    tx_batch_amount_to_swap_token0  = 0.
                    #N.B. If LP_SWAP, reset to 0 session.res_num_tokenX only if priceLP[NUM_LP] is None
                    if (not LP_SWAP) or (LP_SWAP and priceLP[NUM_LP] is None):
                        session.res_num_token0, session.res_num_token1 = 0., 0.
                    ...
                    logger.info(network + ", NO session swap: abs(session.res_num_token0) = " + "{:1.4f}".format(abs(session.res_num_token0)) +\
                    ...
                    if abs(tx_batch_amount_to_swap_token0) >= MIN_TX_BATCH_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price:
                        ...
                        session.res_num_token0 -= tx_batch_amount_to_swap_token0
                        session.res_num_token1 += tx_batch_amount_to_swap_token0 / price
                        tx_batch_amount_to_swap_token0 = 0.
            ...
            if swap_condition:
                if abs(session.amount_to_swap_token0) >= SWAP_EPSILON_PER / 100  * NUM_INVESTED_TOKEN1_LP * price:
                    logger.info(network + ", swap because swap_condition is raised and session.amount_to_swap_token0 = " +\
