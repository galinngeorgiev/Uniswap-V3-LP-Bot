...
def main(network='mumbai'):
    ...
    global NUM_LP
    global UNWIND_DIST_TO_BOUND_PER
    global EVENT_LOGS
    ...
    while i <= main_loop_end:
        ...
        if sum(session.count_LP[:NUM_LP]) > 0 and priceLP[NUM_LP] is None and count_times_increase_NUM_LP < MAX_TIMES_INCREASE_NUM_LP:
            if ((session.count_LP_swap - session.count_LP_swap_failed) + session.count_conventional_swaps) / sum(session.count_LP[:NUM_LP]) >= 0.5:
                ...
                #N.B. append() with list comprehension results in a list being included!
                UNWIND_DIST_TO_BOUND_PER = UNWIND_DIST_TO_BOUND_PER + [(t[0] - NUM_LP * 0.2, t[1]) for t in UNWIND_DIST_TO_BOUND_PER]
                NUM_LP *= 2
        ...
        if (not flag_failed_tx) or (not flag_mint):
            ...
            if isinstance(result, list):
                ...
                if len(session.token0_price_collected) > 0:
                    session.token0_pl_token1 = sum(- s[0] * (price / s[1] - 1.) / price for s in session.token0_price_collected)
                if len(session.token0_price_invested) > 0:
                    session.token0_pl_token1 -= sum(- s[0] * (price / s[1] - 1.) / price for s in session.token0_price_invested)
...
-                             " >= " + str(MAX_FAILED_TX_GAS_USED)  + ' ' + init.network_coin_symbol +  ", stop main()!")
                ...
                if mint_condition[j]:
                    ...
                    if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                        ...
                        if isinstance(result, tuple):
                            ...
                            session.token0_price_invested.append((amount0 / 10**init.token0_decimal, price))
                            session.token1_price_invested.append((amount1 / 10**init.token1_decimal, price))
        ...
        unwind_distance_to_bound = [(UNWIND_DIST_TO_BOUND_PER[k][0] / 100, UNWIND_DIST_TO_BOUND_PER[k][1] / 100) \
                                    if LP_position_to_init_price[k] == 1 else (UNWIND_DIST_TO_BOUND_PER[k][1] / 100,\
                                    UNWIND_DIST_TO_BOUND_PER[k][0] / 100) for k in range(NUM_LP)]                            
                ...
                if abs(session.amount_to_swap_token0) >= SWAP_EPSILON_PER / 100  * NUM_INVESTED_TOKEN1_LP * price:
                    ...
                    #conventional swap
                        ...
                        if isinstance(result, tuple):
                            ...
                            if session.amount_to_swap_token0 > 0:
                                session.conventional_swapped = tuple(map(sum, zip(session.conventional_swapped, (session.amount_to_swap_token0, 0.))))
                            elif session.amount_to_swap_token0 < 0:
                                session.conventional_swapped = tuple(map(sum, zip(session.conventional_swapped, (0., -session.amount_to_swap_token0))))
