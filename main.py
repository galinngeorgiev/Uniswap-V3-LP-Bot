...
def main(network='mumbai'):
    ...
    session_count_LP_swap, session_count_LP_swap_failed, session_count_unwind_stop_profit, session_count_unwind_stop_loss, \
            ...
            session_count_hedge_RL, session_count_non_LP_swaps, session_count_unwind_flow, session_count_success_tx, session_count_failed_tx =\
            0, 0, 0, 0, 0, 0, 0, 0, 0, (NUM_LP + 1) * [(0, 0, 0, 0)] + [(0,)], (NUM_LP + 1) * [(0, 0, 0, 0)] + [(0,)]
    session_LP_fees_bp, session_RL_bp, session_OTM_loss_bp =\
            (NUM_LP + 1) * [0.], (NUM_LP + 1) * [0.], (NUM_LP + 1) * [0.]
    session_token0_rf_pl_token1, session_token0_rf_pl_bp, session_token1_rf_pl_token1, session_token1_rf_pl_bp =\
            (NUM_LP + 1) * [0.], (NUM_LP + 1) * [0.], (NUM_LP + 1) * [0.], (NUM_LP + 1) * [0.]
    session_success_tx_gas_fees, session_failed_tx_gas_fees = (NUM_LP + 1) * [(0., 0., 0., 0.)] + [(0.,)], (NUM_LP + 1) * [(0., 0., 0., 0.)] + [(0.,)]
    ...
    flag_unwind, flag_failed_tx, count_failed_tx = False, False, 0
    ...
    while i <= main_loop_end:
        ...
        if tokenIds[:NUM_LP] == NUM_LP * [False]:
            flag_unwind = False
        ...
        if (not flag_failed_tx) and (flag_change_tokenIds or i % PERIOD_CURRENT_LP_POSITIONS_ITERATIONS == 0):
            ...
            if isinstance(result, list):
                ...
                if len(result) != len([tokenIds[j] for j in range(len(tokenIds)) if tokenIds[j] is not None]) or\
                    (LP_SWAP and len([t for t in tokenIds if t is not None]) > NUM_LP + 1) or\
                    (not LP_SWAP and len([t for t in tokenIds if t is not None]) > NUM_LP):
                    logger.info('')
                    if len(result) != len([tokenIds[j] for j in range(len(tokenIds)) if tokenIds[j] is not None]):
                        logger.error(network + ', len(result) != len(not None tokenIds), flag_unwind is raised!')
                    if LP_SWAP:
                        logger.error(network + ', more than ' + str(NUM_LP + 1) + ' simultaneous LP positions are NOT allowed, burn the extra with burn.py! flag_unwind is raised!')
                    else:
                        logger.error(network + ', more than ' + str(NUM_LP) + ' simultaneous LP positions are NOT allowed, burn the extra with burn.py! flag_unwind is raised!')
                    flag_unwind = True
        ...
        if (not flag_failed_tx) or (not flag_mint):
            ...
            if isinstance(result, list):
                ...
                if price is None:
                    price_list.pop(-1)
                    if i < int(RUNTIME_SEC / DELAY_LOOP_SEC) - NUM_LP * MAX_ATTEMPS_FAILED_TX:
                        logger.info(network + ', price is None, remove None from price_list, i before end of loop, next iteration...')
                        i += 1
                        continue
                elif price == 0.:
                    #price_list.pop(-1) #N.B. price = 0. is not added to price_list!
                    if i < int(RUNTIME_SEC / DELAY_LOOP_SEC) - NUM_LP * MAX_ATTEMPS_FAILED_TX:
                        logger.info(network + ', price = 0., i before end of loop, next iteration...')
                        i += 1
                        continue
                else:
                ...
                if not flag_failed_tx and i < int(RUNTIME_SEC / DELAY_LOOP_SEC) - NUM_LP * MAX_ATTEMPS_FAILED_TX:
                ...
                if priceLP[j] is None:
                    ...
                    if LP_position_to_init_price[j] == 1:
                        if LP_distance_to_bound == 0.:
                            if j == 0:
                                priceLower[j] = price_DECIMAL * (1. + 3 * LP_BOUND_DISTANCE_TO_CURRENT_PER / 100)
                                #N.B. Re-compute priceLower, priceUpper: tick spacing >> 1, depending on the pool fee: section 4 in https://uniswap.org/whitepaper-v3.pdf
                                #N.B. tick = log(price) / log(1.0001): (6.1) in https://uniswap.org/whitepaper-v3.pdf
                            ...
                            priceUpper[j] = price_DECIMAL * (1. + (j + 2) * 2 * LP_distance_to_bound + LP_BOUND_DISTANCE_TO_CURRENT_PER / 100)
                    elif LP_position_to_init_price[j] == -1:
                        if LP_distance_to_bound == 0.:
                            if j == 0:
                                priceUpper[j] = price_DECIMAL * (1. - 3 * LP_BOUND_DISTANCE_TO_CURRENT_PER / 100)
                                #N.B. Re-compute priceLower, priceUpper: tick spacing >> 1, depending on the pool fee: section 4 in https://uniswap.org/whitepaper-v3.pdf
                                #N.B. tick = log(price) / log(1.0001): (6.1) in https://uniswap.org/whitepaper-v3.pdf
                            ...
                            priceLower[j] = price_DECIMAL * (1. - (j + 2) * 2 * LP_distance_to_bound - LP_BOUND_DISTANCE_TO_CURRENT_PER / 100)
                ...
                if mint_condition[j]:
                    ...
                    if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                        ...
                        if isinstance(result, tuple):
                            ...
                            session_count_success_tx[j] = tuple(map(sum, zip(session_count_success_tx[j], (1, 0, 0, 0) )))
                            session_success_tx_gas_fees[j] = tuple(map(sum, zip(session_success_tx_gas_fees[j], (gas_fee, 0., 0., 0.) )))
                            ...
                            if isinstance(result, list):
                                ...
                                session_failed_tx_gas_fees[j] = tuple(map(sum, zip(session_failed_tx_gas_fees[j], (gas_fee, 0., 0., 0.) )))
                            session_count_failed_tx[j] = tuple(map(sum, zip(session_count_failed_tx[j], (1, 0, 0, 0) )))
        ...
        if tokenIds != (NUM_LP + 1) * [None]:
            ...
            #N.B. if not "if flag_mint", if mint() fails, j becomes to large here and mint_condition[j] = True fails with error "list assignment index out of range"!
            if flag_mint:
                if not flag_failed_tx:
                    j = 0
                while j <= NUM_LP:
                    ...
                    if unwind_condition[j] and tokenIds[j] is not None:
                        ...
                        result = positions(network, tokenIds[j])
                        if isinstance(result, tuple):
                            tickLower[j], tickUpper[j], liquidities[j] = result
                        else:
                            logger.info('')
                            logger.info(network + ', positions() failed, use liquidities[] in code!')  
                        ...
                        if flag_increaseLiquidity and flag_hedgeRL and flag_collect and flag_burn:
                            ...
                            if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                                ...
                                if isinstance(result, tuple):
                                    ...
                                    session_count_success_tx[j] = tuple(map(sum, zip(session_count_success_tx[j], (0, 1, 0, 0) )))
                                    session_success_tx_gas_fees[j] = tuple(map(sum, zip(session_success_tx_gas_fees[j], (0., gas_fee, 0., 0.) )))
                                    ...
                                    if isinstance(result, list):
                                        [gas_fee, status] = result
                                        session_failed_tx_gas_fees[j] = tuple(map(sum, zip(session_failed_tx_gas_fees[j], (0., gas_fee, 0., 0.) )))
                                    session_count_failed_tx[j] = tuple(map(sum, zip(session_count_failed_tx[j], (0, 1, 0, 0) )))
                                    #N.B. nonce=None triggers web3 getting a nonce
                                ...
                                logger.info(network + ", session: " + init.token1_symbol + "-ref-frame p&l w/o swaps = " +\
                                    ...
                                    ", token0 collected in " + init.token1_symbol + "-ref-frame p&l = " +\
                                            "{:1.4f}".format(session_token0_pl_token1_rf_token1) + ' ' + init.token1_symbol + ' or '+\
                                            "{:1.2f}".format(session_token0_pl_token1_rf_token1 / NUM_INVESTED_TOKEN1_LP * 10000) + ' bp')
                                logger.info(network + ", session: count success tx-s = " + "{:d}".format(sum([sum(s) for s in session_count_success_tx])) +\
                                    " = [" + str(["{:d}".format(s[0]) for s in session_count_success_tx[:-1]]) +\
                                    ", " + str(["{:d}".format(s[1]) for s in session_count_success_tx[:-1]]) +\
                                    ", " + str(["{:d}".format(s[2]) for s in session_count_success_tx[:-1]]) +\
                                    ", " + str(["{:d}".format(s[3]) for s in session_count_success_tx[:-1]]) +\
                                    ", " + "{:d}".format(session_count_success_tx[-1][0]) + '] ')
                                logger.info(network + ", session: count failed  tx-s = " + "{:d}".format(sum([sum(s) for s in session_count_failed_tx])) +\
                                    " = [" + str(["{:d}".format(s[0]) for s in session_count_failed_tx[:-1]]) +\
                                    ", " + str(["{:d}".format(s[1]) for s in session_count_failed_tx[:-1]]) +\
                                    ", " + str(["{:d}".format(s[2]) for s in session_count_failed_tx[:-1]]) +\
                                    ", " + str(["{:d}".format(s[3]) for s in session_count_failed_tx[:-1]]) +\
                                    ", " + "{:d}".format(session_count_failed_tx[-1][0]) + '] ')
                                logger.info(network + ", session: gas fees on success tx-s = " +\
                                    "{:1.2f}".format(sum([sum(s) for s in session_success_tx_gas_fees])) + ' ' + init.network_coin_symbol +\
                                    " = [" + str(["{:1.2f}".format(s[0]) for s in session_success_tx_gas_fees[:-1]]) +\
                                    ", " + str(["{:1.2f}".format(s[1]) for s in session_success_tx_gas_fees[:-1]]) +\
                                    ", " + str(["{:1.2f}".format(s[2]) for s in session_success_tx_gas_fees[:-1]]) +\
                                    ", " + str(["{:1.2f}".format(s[3]) for s in session_success_tx_gas_fees[:-1]]) +\
                                    ", " + "{:1.2f}".format(session_success_tx_gas_fees[-1][0]) + '] ' + init.network_coin_symbol)
                                logger.info(network + ", session: gas fees on failed  tx-s = " +\
                                    "{:1.2f}".format(sum([sum(s) for s in session_failed_tx_gas_fees])) + ' ' + init.network_coin_symbol +\
                                    " = [" + str(["{:1.2f}".format(s[0]) for s in session_failed_tx_gas_fees[:-1]]) +\
                                    ", " + str(["{:1.2f}".format(s[1]) for s in session_failed_tx_gas_fees[:-1]]) +\
                                    ", " + str(["{:1.2f}".format(s[2]) for s in session_failed_tx_gas_fees[:-1]]) +\
                                    ", " + str(["{:1.2f}".format(s[3]) for s in session_failed_tx_gas_fees[:-1]]) +\
                                    ", " + "{:1.2f}".format(session_failed_tx_gas_fees[-1][0]) + '] ' + init.network_coin_symbol)
                                ...
                                logger.info(network + ', session' +\
                                ': count non-swap LP=' + str(sum(session_count_LP[:NUM_LP])) +\
                                ', count swap LP=' + str(session_count_LP[NUM_LP]) +\
                                ', count swap LP failed=' + str(session_count_LP_swap_failed) +\
                                ', count conventional swaps=' + str(session_count_non_LP_swaps) +\
                        ...
                        if flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_burn:
                            if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                                ...
                                if isinstance(result, tuple):
                                    gas_fee, amount0, amount1, nonce = result
                                    blockNumber_init, price_init = block_num, price #if signed_token1_quantity > 0 else -price
                                    session_count_success_tx[j] = tuple(map(sum, zip(session_count_success_tx[j], (0, 0, 1, 0) )))
                                    session_success_tx_gas_fees[j] = tuple(map(sum, zip(session_success_tx_gas_fees[j], (0., 0., gas_fee, 0.) )))
                                    ...
                                    if isinstance(result, list):
                                        [gas_fee, status] = result
                                        session_failed_tx_gas_fees[j] = tuple(map(sum, zip(session_failed_tx_gas_fees[j], (0., 0., gas_fee, 0.) )))
                                    session_count_failed_tx[j] = tuple(map(sum, zip(session_count_failed_tx[j], (0, 0, 1, 0) )))
                                    #N.B. nonce=None triggers web3 getting a nonce
                        ...
                        if flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect:
                            ...
                            if count_failed_tx < MAX_ATTEMPS_FAILED_TX:                        
                                ...
                                if isinstance(result, tuple):
                                    ...
                                    session_count_success_tx[j] = tuple(map(sum, zip(session_count_success_tx[j], (0, 0, 0, 1) )))
                                    session_success_tx_gas_fees[j] = tuple(map(sum, zip(session_success_tx_gas_fees[j], (0., 0., 0., gas_fee) )))
                                    tokenIds_burned.append(tokenIds[j])
                                    nonce += 1
                                    flag_change_tokenIds = True
                                    logger.error(network + ", NFT token with ID " + str(tokenIds[j]) + " is burned. Burned tokens = " + str(tokenIds_burned))
                                    ...
                                    if isinstance(result, list):
                                        [gas_fee, status] = result
                                        session_failed_tx_gas_fees[j] = tuple(map(sum, zip(session_failed_tx_gas_fees[j], (0., 0., 0., gas_fee) )))
                                    session_count_failed_tx[j] = tuple(map(sum, zip(session_count_failed_tx[j], (0, 0, 0, 1) )))
                                    #N.B. nonce=None triggers web3 getting a nonce
        ...
        ########################################################################
        #N.B. if unwind_condition[] is raised, log-reporting p&l for all trades
        if not flag_failed_tx:
        ...
        if flag_mint and flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect and flag_burn:
            ...
            if flag_LP_swap and (count_LP_swap_attempts >= LP_SWAP_MAX_ATTEMPTS_FAILED_TX or i >= main_loop_end - MAX_ATTEMPS_FAILED_TX):
                ...
                if abs(session_res_num_token0) >= MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price or\
                    ...
                    tx_batch_amount_to_swap_token0  = 0.
                    #N.B. If LP_SWAP, reset to 0 session_res_num_tokenX only if priceLP[NUM_LP] is None
                    if (not LP_SWAP) or (LP_SWAP and priceLP[NUM_LP] is None):
                    ...
                    logger.info(network + ", NO session swap: abs(session_res_num_token0) = " + "{:1.4f}".format(abs(session_res_num_token0)) +\
                            " <= min session swap token1 = " + str(MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price)  + ' ' + init.token0_symbol)
                    #N.B. tx batch swap
                ...
                if abs(amount_to_swap_token0) >= SWAP_EPSILON_PER / 100  * NUM_INVESTED_TOKEN1_LP * price:
                    ...
                    #conventional swap
                        ...
                        if isinstance(result, tuple):
                            ...
                            session_count_success_tx[-1] = tuple(map(sum, zip(session_count_success_tx[-1], (1, ) )))
                            session_success_tx_gas_fees[-1] = tuple(map(sum, zip(session_success_tx_gas_fees[-1], (gas_fee, ) )))
                            ...
                            session_count_failed_tx[-1] = tuple(map(sum, zip(session_count_failed_tx[-1], (1, ) )))
                            nonce = None
                            if result:
                                i += 1
                                continue
                                ...
                                if isinstance(result, list):
                                    [gas_fee, status] = result
                                    session_failed_tx_gas_fees[-1] = tuple(map(sum, zip(session_failed_tx_gas_fees[-1], (gas_fee, ) )))
                                logger.info('')
