...
def main(network='mumbai'):
    ...
    session_token0_price_collected, session_token1_price_collected, session_token0_collected_token1_rf_pl_token1, amount_to_swap_token0 = [], [], 0., 0.
    ...
    while i <= main_loop_end:
                ...
                if len([t for t in tokenIds if t is not None]) > NUM_LP + 1:
                    ...
                    logger.info(network + ", session: " + init.token1_symbol + "-ref-frame p&l w/o swaps = " +\
                        ...
                        ", token0 collected in " + init.token1_symbol + "-ref-frame p&l = " +\
                                "{:1.4f}".format(session_token0_collected_token1_rf_pl_token1) + ' ' + init.token1_symbol + ' or '+\
                                "{:1.2f}".format(session_token0_collected_token1_rf_pl_token1 / NUM_INVESTED_TOKEN1_LP * 10000) + ' bp')
                    logger.info(network + ", session: gas fees=" + "{:1.2f}".format(sum([sum(s) for s in session_gas_fees]))  +\
            ...
            if isinstance(result, list):
                ...
                if price is None:
                    ...
                    if len(session_token0_price_collected) > 0:
                        session_token0_collected_token1_rf_pl_token1 = sum(- s[0] * (price / s[1] - 1.) / price for s in session_token0_price_collected)
            ...
            if not flag_mint:
                ...
                while j < NUM_LP:
                    if tokenIds[j] is None:
                        ...
                        #    else:
                        #        #N.B. Change the relative position (to price) of the OTM LP
                        ...
                        if LP_position_to_init_price[j] == 1:
                            priceLower[j] = price_DECIMAL * (1. + (j + 0) * 2 * LP_distance_to_bound + LP_SWAP_DISTANCE_TO_BOUND_PER / 100)
                            ...
                            priceUpper[j] = price_DECIMAL * (1. - (j + 0) * 2 * LP_distance_to_bound - LP_SWAP_DISTANCE_TO_BOUND_PER / 100)
                        ...
                        if init.pool_fee == 500:
                            ...
                            #tickLower[j], tickUpper[j] = int(tickLower[j] // 10 * 10), int(tickUpper[j] // 10 * 10 + 6 * 10)
                            tickLower[j], tickUpper[j] = int(tickLower[j] // 10 * 10), int(tickUpper[j] // 10 * 10) # + 10)
                        elif init.pool_fee == 3000:
                            tickLower[j], tickUpper[j] = int(tickLower[j] // 60 * 60), int(tickUpper[j] // 60 * 60) # + 60)
                        elif init.pool_fee == 10000:
                            tickLower[j], tickUpper[j] = int(tickLower[j] // 200 * 200), int(tickUpper[j] // 200 * 200) # + 200)
                        ...
                        if i < int(RUNTIME_SEC / DELAY_LOOP_SEC) - NUM_LP * MAX_ATTEMPS_FAILED_TX: #tokenIds[:NUM_LP] == NUM_LP * [None]
                            ...
                            if i > 0.75 * int(RUNTIME_SEC / DELAY_LOOP_SEC) and LP_position_to_init_price[j] == INIT_LP_POSITION_TO_INIT_PRICE:
        ...
        unwind_distance_to_bound = [(UNWIND_DIST_TO_BOUND_PER[j][0] / 100, UNWIND_DIST_TO_BOUND_PER[j][1] / 100) \
                                    if LP_position_to_init_price[j] == 1 else (UNWIND_DIST_TO_BOUND_PER[j][1] / 100, UNWIND_DIST_TO_BOUND_PER[j][0] / 100) \
        ...
        if tokenIds != (NUM_LP + 1) * [None]:
            if not flag_failed_tx:
                ...
                if flag_mint:
                    ...
                    #N.B. Treat non-swap LP-s (still under 'not flag_failed_tx'; othewise 'unwind_condition' is not touched!)
                            ...
                            if tokenIds[j] is not None and priceLower[j] is not None and priceUpper[j] is not None and priceLP[j] is not None:
                                ...
                                if price_DECIMAL >= priceLower[j] and price_DECIMAL <= priceUpper[j]:
                                    ...
                                    if EVENT_LOGS and (j + 1 == NUM_LP or tokenIds[j + 1] == None) and\
                                            LP_position_to_init_price[j] * signed_token1_quantity / pool_liquidity_list[-1]  *\
                                            10 ** (0.5 * (init.token0_decimal + init.token1_decimal)) * 10000 > MAX_UNWIND_TOKEN1_QUANTITY_TO_TVL_BP:
                                        logger.info('')
                                        logger.info(network + ", UNWIND LP[" + str(j) + "] position with tokenId = " + str(tokenIds[j]) +\
                                                            " because LP_position_to_init_price[j] * signed_token1_quantity / pool_liquidity * 10000 = " +\
                                                            "{:1.4f}".format(LP_position_to_init_price[j] * signed_token1_quantity / pool_liquidity_list[-1] *\
                            ...
                            if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                                ...
                                if isinstance(result, tuple):
                                    ...
                                    #N.B. tx_batch_amount_to_swap_token1: swap tx_batch_amount_to_swap_token1*price token0 into token1 if > 0 or -tx_batch_amount_to_swap_token1 token1 into token0 if < 0
                                        tx_batch_amount_to_swap_token1 += (amount0 - amount0_invested[j]) / 10**init.token0_decimal / price
        ...
        if count_LP_swap_attempts >= LP_SWAP_MAX_ATTEMPTS_FAILED_TX or i == main_loop_end - MAX_ATTEMPS_FAILED_TX or \
            ...
            if flag_LP_swap and (count_LP_swap_attempts >= LP_SWAP_MAX_ATTEMPTS_FAILED_TX or i == main_loop_end - MAX_ATTEMPS_FAILED_TX):
                ...
                non_LP_swap_condition = True
            else:
                amount_to_swap_token0 = 0.
                non_LP_swap_condition = False
                ...
                if abs(session_res_num_token1) > MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP or\
                    ...
                    logger.info(network + ", no session swap: abs(session_res_num_token1)=" + str(abs(session_res_num_token1)) +\
                            " <= min session swap token1=" + str(MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP))
                    #N.B. tx batch swap
                    ...
                    if abs(tx_batch_amount_to_swap_token1) > MIN_TX_BATCH_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP:
                        ...
                        logger.info(network + ", no tx batch swap either: abs(tx_batch_amount_to_swap_token1) = " + str(abs(tx_batch_amount_to_swap_token1)) +\
                           " <= min tx swap threshold = " + str(MIN_TX_BATCH_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP) + ", continue with next iter...")
                        flag_swap_tx_batch, flag_swap_session = False, False
                        non_LP_swap_condition = False
                    

            #non-LP swap()
            if non_LP_swap_condition and abs(amount_to_swap_token0) > SWAP_EPSILON_PER / 100  * NUM_INVESTED_TOKEN1_LP * price :
                logger.info(network + ", non_LP_swap_condition is raised and amount_to_swap_token0 = " + str(amount_to_swap_token0) + " with abs(amount_to_swap_token0) > " +\
                ...
                if LP_SWAP and count_LP_swap_attempts < LP_SWAP_MAX_ATTEMPTS_FAILED_TX and i < main_loop_end - MAX_ATTEMPS_FAILED_TX:
                    ...
                    if isinstance(result, tuple):
                        ...
                        #N.B. tx
                        tx_slippage_and_pool_fee_pl_bp += slippage_and_pool_fee_token1 / NUM_INVESTED_TOKEN1_LP * 10000 if\
                                NUM_INVESTED_TOKEN1_LP > 0 else 0.
