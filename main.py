            ...
            if isinstance(result, tuple):
                ...
                if TOKEN_ALCHEMY:
                    ...
                    if [t for t in burn_time if t is not None] != [] and\
                        ...
                        logger.info(network + ', TOKEN_ALCHEMY = True but using tokenIds_burned from API = ' + str(tokenIds_burned))
                    ...
                    if unwind_condition[j] and tokenIds[j] is not None:
                        if not flag_failed_tx:
                            ...
                            if isinstance(result, tuple):
                                ...
                                max_fee_per_gas, max_priority_fee_per_gas = int(MULT_GAS_FACTOR_REPLACE * max_fee_per_gas * 1e9),\
                                                                            int(MULT_GAS_FACTOR_REPLACE * max_priority_fee_per_gas * 1e9)
                                logger.info(network + ", use 99-th percentile of current gas: max_fee_per_gas = " +\
                                   str(MULT_GAS_FACTOR_REPLACE) + ' * ' + "{:1.2f}".format(max_fee_per_gas / 1e9) + ' Gwei' +\
                                    ', max_priority_fee_per_gas = ' +\
                                   str(MULT_GAS_FACTOR_REPLACE) + ' * ' + "{:1.2f}".format(max_priority_fee_per_gas / 1e9) + ' Gwei')
                            else:
        ...
        if flag_mint and flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect and flag_burn:
            ...
            if flag_LP_swap and (count_LP_swap_attempts >= LP_SWAP_MAX_ATTEMPTS_FAILED_TX or i >= main_loop_end - MAX_ATTEMPS_FAILED_TX):
                ...
                if abs(session.res_num_token0) >= MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price or\
                    ...
                    logger.info(network + ", NO session swap: abs(session.res_num_token0) = " +\
                            "{:1.2f}".format(abs(session.res_num_token0)) +\
                            ' ' + init.token0_symbol + " <= min session swap token1 = " +\
                            str(MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price)  + ' ' + init.token0_symbol)
                    #N.B. tx batch swap
                    ...
                    #conventional swap
                        ...
                        if not flag_failed_tx:
                            ...
                            if isinstance(result, tuple):
                                ...
                                max_fee_per_gas, max_priority_fee_per_gas = int(MULT_GAS_FACTOR_REPLACE * max_fee_per_gas * 1e9),\
                                                                            int(MULT_GAS_FACTOR_REPLACE * max_priority_fee_per_gas * 1e9)
                                logger.info(network + ", use MULT_GAS_FACTOR_REPLACE * 99-th percentile of current gas: max_fee_per_gas = " +\
                                   str(MULT_GAS_FACTOR_REPLACE) + ' * ' + '{:1.2f}'.format(max_fee_per_gas / 1e9) + ' Gwei' +\
                                    ', max_priority_fee_per_gas = ' +\
                                   str(MULT_GAS_FACTOR_REPLACE) + ' * ' + "{:1.2f}".format(max_priority_fee_per_gas / 1e9) + ' Gwei')
                            else:
