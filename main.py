...
def main(network='mumbai'):
    ...
    result = current_pool_price(network, block_num, CEX_price_list, price_list, price_list_median, pool_liquidity_list, pool_liquidity_median, \
    ...
    while i <= main_loop_end:
        ...
        if sum(session.count_LP[:NUM_LP]) > 2 * NUM_LP and priceLP[NUM_LP] is None and count_times_increase_NUM_LP < MAX_TIMES_INCREASE_NUM_LP:
            if ((session.count_LP_swap - session.count_LP_swap_failed) + session.count_conventional_swaps) / sum(session.count_LP[:NUM_LP]) >= 0.5:
        ...
        if (not flag_failed_tx) or (not flag_mint) or flag_unwind:
            ...
            if isinstance(result, list):
                ...
                #N.B. update lp_bound_distance_to_current_per at every iteration, when current_pool_price() succeeds
        ...
        if (not flag_unwind) and flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect and flag_burn:
            ...
            if flag_mint:
                ...
                #N.B. Do not initiate if the potential mint() will exceed NUM_INVESTED_TOKEN1_LP
            ...
            if price_list[-1] is None:
                mint_condition = ((MAX_TIMES_INCREASE_NUM_LP + 1) * NUM_LP + 1) * [False]
            ...
            if any(mint_condition) and (not flag_failed_tx):
                ...
                if GAS_EXT:
                    ...
                    if isinstance(result, tuple):
                        ...
                        logger.info(network + ", mint() uses 99-th percentile gas in blocknative.com: max_fee_per_gas = " +\
            ...
            if unwind_condition != ((MAX_TIMES_INCREASE_NUM_LP + 1) * NUM_LP + 1) * [False] and (flag_mint or flag_unwind):
                ...
                if not flag_failed_tx:
                    ...
                    if GAS_EXT:
                        ...
                        if isinstance(result, tuple):
                            max_fee_per_gas, max_priority_fee_per_gas = result
                            logger.info(network + ", decreaseLiquidity() & collect() use 99-th percentile gas in blocknative.com: max_fee_per_gas = " +\
                                "{:1.2f}".format(max_fee_per_gas) + ' Gwei' +\
                                ', max_priority_fee_per_gas = ' +\
                                "{:1.2f}".format(max_priority_fee_per_gas) + ' Gwei')
                            max_fee_per_gas, max_priority_fee_per_gas = int(max_fee_per_gas * 1e9),\
        ...
        ########################################################################
        #7. log-reporting p&l for all trades
        ...
        ######################################################################
        #8. Swap the collected amounts difference to achieve the original amounts;
                    ...
                    #conventional swap
                        ...
                        if not flag_failed_tx:
                            ...
                            if GAS_EXT:
                                ...
                                if isinstance(result, tuple):
                                    max_fee_per_gas, max_priority_fee_per_gas = result
                                    logger.info(network + ", swap() uses INIT_SWAP_MULT_GAS_FACTOR_REPLACE * 99-th percentile of gas in blocknative.com: max_fee_per_gas = " +\
        ...
        if unwind_condition != ((MAX_TIMES_INCREASE_NUM_LP + 1) * NUM_LP + 1) * [False] and\
            ...
            if not flag_failed_tx:
                ...
                if GAS_EXT:
                    ...
                    if isinstance(result, tuple):
                        max_fee_per_gas, max_priority_fee_per_gas = result
                        logger.info(network + ", burn() uses 99-th percentile gas in blocknative.com: max_fee_per_gas = " +\
                            "{:1.2f}".format(max_fee_per_gas) + ' Gwei' +\
                            ', max_priority_fee_per_gas = ' +\
                            "{:1.2f}".format(max_priority_fee_per_gas) + ' Gwei')
                        max_fee_per_gas, max_priority_fee_per_gas = int(max_fee_per_gas * 1e9),\
