...
def main(network='mumbai'):
    ...
    #initialize tx
    i, block_num, price_list, price, price_list_median, price_median, price_mad, max_price_mad_time, swap_price, avg_price_mad, max_price_mad =\
    ...
    result = current_pool_price(network, block_num, price_list, price_list_median, pool_liquidity_list, pool_liquidity_median, \
                price_mad, avg_price_mad, max_price_mad, priceLower, priceUpper, swap_volume_token1, rel_swap_volume_token1, swap_flow_token1, PRICE_ALCHEMY, init=init)      
    if isinstance(result, list):
        ...
        GMTtimeStamp, block_num, txIx, price_list, price_list_median, pool_liquidity_list, pool_liquidity_median, signed_token1_quantity,\
                        price_mad, avg_price_mad, max_price_mad, swap_volume_token1, rel_swap_volume_token1, swap_flow_token1 = result
    ...
    while i <= main_loop_end:
        ...
        if sum(session.count_LP[:NUM_LP]) > 2 * NUM_LP and priceLP[NUM_LP] is None and count_times_increase_NUM_LP < MAX_TIMES_INCREASE_NUM_LP:
            if ((session.count_LP_swap - session.count_LP_swap_failed) + session.count_conventional_swaps) / sum(session.count_LP[:NUM_LP]) >= 0.5:
                ...
                NUM_LP *= 2
                logger.info(network + ', increase NUM_LP to ' + str(NUM_LP) + ' because num swaps / num non-swap LP >= 0.5!')
        
        if tokenIds == (NUM_LP + 1) * [None]:
            logger.info(network + ', flag_unwind = False because tokenIds == (NUM_LP + 1) * [None]')
            flag_unwind = False
                ...
                if mint_condition[j]:
                    ...
                    if count_failed_tx < MAX_ATTEMPS_FAILED_TX:
                        ...
                        if isinstance(result, tuple):
                            ...
                            if isinstance(result, list):
                                GMTtimeStamp, block_num, txIx, price_list, price_list_median, pool_liquidity_list, pool_liquidity_median, signed_token1_quantity,\
                                    price_mad, avg_price_mad, max_price_mad, swap_volume_token1_output, rel_swap_volume_token1_output, swap_flow_token1_output = result
        ...
        if tokenIds != (NUM_LP + 1) * [None]:
            ...
            if flag_unwind:
                unwind_condition = (NUM_LP + 1) * [True]
        ...
        if flag_increaseLiquidity and flag_hedgeRL and flag_decreaseLiquidity and flag_collect and flag_burn:
            ...
            if flag_mint:
                ...
                if abs(amount_to_swap_token0) >= MIN_SESSION_SWAP_PER / 100 * NUM_INVESTED_TOKEN1_LP * price or\
                    ...
                    logger.info(network + ", NO session swap_condition = True: abs(amount_to_swap_token0) = " +\
                ...
                if abs(amount_to_swap_token0) >= SWAP_EPSILON_PER / 100  * NUM_INVESTED_TOKEN1_LP * price:
                    ...
                    #conventional swap
                        ...
                        if not flag_failed_tx:
                            ...
                            if isinstance(result, tuple):
                                max_fee_per_gas, max_priority_fee_per_gas = result
                                logger.info(network + ", use INIT_SWAP_MULT_GAS_FACTOR_REPLACE * 99-th percentile of gas in blocknative.com: max_fee_per_gas = " +\
