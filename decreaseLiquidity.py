...
def decreaseLiquidity(network, tokenId, liquidity, max_fee_per_gas=MAX_FEE_PER_GAS, max_priority_fee_per_gas=MAX_PRIORITY_FEE_PER_GAS,\
    ...
    try:
       ...
       logger.error(network + ', send_raw_transaction() in decreaseLiquidity() failed for tokenId ' + str(tokenId) + '. Try again with higher gas! ' + traceback.format_exc(limit=0))
       return False
    ...
    try:
        ...
        if "Unable to complete request at this time" in str(e) or 'asyncio.exceptions.CancelledError' in str(e):
            logger.error(network + ', wait_for_transaction_receipt() in decreaseLiquidity() failed with error ' + traceback.format_exc(limit=0))
            tx_receipt = delay_error(network, init, tx_hash)
        else:
           logger.error(network + ', wait_for_transaction_receipt() in decreaseLiquidity() failed after TIMEOUT_SEC = ' + str(TIMEOUT_SEC) + ' with error ' + traceback.format_exc(limit=0) + '. Try again with higher gas!')
           return False
    #print(tx_receipt)

    if tx_receipt['status'] == 0:
        logger.error(network + ', decreaseLiquidity() failed with tx_receipt[status] == 0 for tokenId ' + str(tokenId) + '. Tx receipt %s. Try again with higher gas! ', tx_receipt)
        return False
    else:
