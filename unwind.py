...
def unwind_wrapped(network, tokenId, max_fee_per_gas=MAX_FEE_PER_GAS, max_priority_fee_per_gas=MAX_PRIORITY_FEE_PER_GAS):
    ...
    if isinstance(result, tuple):
        time.sleep(10)
        result = collect(network, tokenId, max_fee_per_gas, max_priority_fee_per_gas, address=Address, private_key=privateKey)
        if isinstance(result, tuple):
            time.sleep(10)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 3:  # network, tokenId
        unwind_wrapped(network=str(sys.argv[1]), tokenId=eval(sys.argv[2]))
