...
LP_BOUND_DISTANCE_TO_CURRENT_PER =  [0.1, 0.6] #min (1st) and max (2nd) of lp_bound_distance_to_current_per
...
MAX_INIT_RET_CEFI_DEFI_PER, MAX_UNWIND_RET_CEFI_DEFI_PER = 10000., 0.15
...
if CHANGE_LP_POSITION_TO_INIT_PRICE:
        ...
        LP_SWAP, LP_SWAP_MULT_RUNTIME, LP_SWAP_DISTANCE_TO_BOUND_PER, LP_SWAP_UNWIND_DISTANCE_PER, LP_SWAP_MAX_ATTEMPTS_FAILED_TX = False, 2, 0.1, 0.15, 1
assert LP_SWAP_DISTANCE_TO_BOUND_PER > 0. #N.B. Otherwise, LP swap unwinds!
assert LP_SWAP_UNWIND_DISTANCE_PER  > LP_SWAP_DISTANCE_TO_BOUND_PER #N.B. Otherwise, LP swap unwinds!
...
MAX_ATTEMPS_FAILED_TX, MAX_ATTEMPS_FAILED_PRICE, MAX_ATTEMPS_FAILED_TOKEN = 5, 100, 6
assert MAX_ATTEMPS_FAILED_TX > LP_SWAP_MAX_ATTEMPTS_FAILED_TX
INIT_SWAP_MULT_GAS_FACTOR_REPLACE, MULT_GAS_FACTOR_REPLACE, MAX_MULT_FACTOR_GAS_REPLACE = 2, 1.5, 5
INIT_SLIPPAGE_PER, SLIPPAGE_STEP_PER, MAX_SLIPPAGE_PER = 0.3, 0.1, 0.5 #N.B. If slippage is too low, get error 'Too little received'
DELAY_LOOP_SEC = 0.5
...
EXPIRY_SEC, TIMEOUT_SEC, MINT_EXPIRY_SEC, MINT_TIMEOUT_SEC, RECEIPT_POLLING_SEC = 300, 30, 300, 300, 0.1 #N.B. 1st is used for deadline (after which, cancel) in mint, decreaseLiquidity, swap; 2nd is used in .wait_for_transaction_receipt (if TIMEOUT_SEC=0,  w3.eth.wait_for_transaction_receipt returns error TimeExhausted)
...
MAX_PRICE_RETURN_PER = 0 #N.B. used in current_pool_price(): larger numbers result is bad prices! 
...
COIN_SYMBOL_MAINNET, COIN_DECIMAL_MAINNET, SCAN_MAINNET = 'ETH', 18, 'Etherscan.io'
...
#################
COIN_SYMBOL_POLYGON, COIN_DECIMAL_POLYGON, SCAN_POLYGON = 'MATIC', 18, 'Polygonscan.com'
