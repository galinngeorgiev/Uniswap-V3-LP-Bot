...
if TEST:
	...
	#N.B. NUM_LP >> 0 for the same cumulative price range is beneficial because:
	#N.B. i) LP fees are higher (although LP fees ~ inverse width when range is small) because avg is typically wrong: distibution of ITM durations is not uniform! 
	#N.B. ii) RL is smaller because RL is very non-linear!
	...
	EXPIRY_SEC, TIMEOUT_SEC = 300, 300 #N.B. 1st is used for deadline (after which, cancel) in mint, decreaseLiquidity; 2nd is used in .wait_for_transaction_receipt (if TIMEOUT_SEC=0,  w3.eth.wait_for_transaction_receipt returns error TimeExhausted)
