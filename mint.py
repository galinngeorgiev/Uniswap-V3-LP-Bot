...
#N.B. i) if price is in-range: use the smaller of liquidity from amountToMint0 and liquidity from amountToMint1
#N.B. ii) if price is out-of-range: use either liquidity from amountToMint0 or liquidity from amountToMint1
#N.B. price is for token1 w.r.t. token 0
def mint(network='mumbai', amount0ToMint=0.02*10**TOKEN0_DECIMAL_MUMBAI, amount1ToMint=0.0001*10**TOKEN1_DECIMAL_MUMBAI,\
            tickLower=0, tickUpper=0, max_fee_per_gas=MAX_FEE_PER_GAS, max_priority_fee_per_gas=MAX_PRIORITY_FEE_PER_GAS,\
    ...
    expiryDate = int(time.time()) + MINT_EXPIRY_SEC
    #try:
    #    expiryDate = init.w3.eth.get_block('latest')["timestamp"] + MINT_EXPIRY_SEC
    #except:
    #    logger.error(network + ', expiryDate in mint() failed. Next main() iteration (with higher gas)!')
    #    return False
   
    params = (
                init.token0_address, #token0
                init.token1_address, #token1
				init.pool_fee, #fee
                -tickUpper, #tickLower (for token0) 
                -tickLower, #tickUpper (for token0)
    ...
    #N.B. The nonce does not use 'pending', so the 2nd tx (with higher gas) cancels & replaces a pending tx
    try:
    ...
    lp_tx = c.contract.functions.mint(params).build_transaction({
    ...
    try:
        ...
        #    logger.error(network + ', send_raw_transaction() in mint() failed with error ' + traceback.format_exc(limit=0))
        #    tx_hash = delay_error(network, init, 'hash', signed_tx.rawTransaction)
        #else:
        logger.error(network + ', send_raw_transaction() in mint() failed with error ' + traceback.format_exc(limit=0) + '. Try again with higher gas! ' )
        return False
    ...
    try:
        ...
        tx_receipt = init.w3.eth.wait_for_transaction_receipt(tx_hash, timeout = MINT_TIMEOUT_SEC)
        ...
        if "Unable to complete request at this time" in traceback.format_exc() or 'CancelledError' in traceback.format_exc() or 'IncompleteReadError' in traceback.format_exc():
            #N.B. wait!
            logger.error(network + ', wait_for_transaction_receipt() in mint() failed with error ' + traceback.format_exc(limit=0))
            tx_receipt = delay_error(network, init, 'receipt', tx_hash)
        else:
            logger.error(network + ', wait_for_transaction_receipt() in mint() failed after TIMEOUT_SEC = ' + str(TIMEOUT_SEC) + ' with error ' + traceback.format_exc(limit=0) + '. Try again with higher gas!')
            return False
       
    if tx_receipt['status'] == 0:
        logger.error(network + ', mint() failed with tx_receipt[status] == 0! Tx receipt %s. Try again with higher gas!', tx_receipt)
        ...
        logger.info(network + ', mint() succedeed: block=' + str(blockNumber) +\
    ...
    return gas_fee, tokenId, liquidity, amount0, amount1, blockNumber, nonce


def mint_wrapped(network='mumbai', amount0ToMint=0.02*10**TOKEN0_DECIMAL_MUMBAI, amount1ToMint=0.0001*10**TOKEN1_DECIMAL_MUMBAI, tickLower=0, tickUpper=0, max_fee_per_gas=MAX_FEE_PER_GAS, max_priority_fee_per_gas=MAX_PRIORITY_FEE_PER_GAS): 
    ...
    mint(network, amount0ToMint, amount1ToMint, tickLower, tickUpper, max_fee_per_gas, max_priority_fee_per_gas, address=Address, private_key=privateKey)

if __name__ == '__main__':
    if len(sys.argv) == 5:  # network, amount0ToMint, amount1ToMint, price_lower, price_upper
        mint_wrapped(network=str(sys.argv[1]), amount0ToMint=eval(sys.argv[2]), amount1ToMint=eval(sys.argv[3]), tickLower=eval(sys.argv[4]), tickUpper=eval(sys.argv[5]))
    elif len(sys.argv) == 7:  # network, max_fee_per_gas, max_priority_fee_per_gas
        mint_wrapped(network=str(sys.argv[1]), amount0ToMint=eval(sys.argv[2]), amount1ToMint=eval(sys.argv[3]), tickLower=eval(sys.argv[4]), tickUpper=eval(sys.argv[5]), max_fee_per_gas=eval(sys.argv[6]), max_priority_fee_per_gas=eval(sys.argv[7]))
    else:   
