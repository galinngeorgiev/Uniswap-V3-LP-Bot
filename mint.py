...
def mint(network='mumbai', amount0ToMint=0.02*10**TOKEN0_DECIMAL_MUMBAI, amount1ToMint=0.0001*10**TOKEN1_DECIMAL_MUMBAI,\
            price_lower=500, price_upper=2000, max_fee_per_gas=MAX_FEE_PER_GAS, max_priority_fee_per_gas=MAX_PRIORITY_FEE_PER_GAS,\
    ...
    tickLower = np.log(price_lower) / np.log(1.0001)
    tickUpper = np.log(price_upper) / np.log(1.0001)
    ...
    try:
        ...
        if "Unable to complete request at this time" in str(e) or 'asyncio.exceptions.CancelledError' in str(e):
            ...
            tx_receipt = delay_error(network, init, tx_hash)
        else:
    ...
    if tx_receipt['status'] == 0:
        ...
        logger.info(network + ', mint() succedeed: block=' + str(blockNumber) +\
           ', tokenId=' + str(tokenId) + ', liquidity=' + "{:1.4f}".format(liquidity / 10 ** (0.5 * (init.token0_decimal + init.token1_decimal))) +\
...
def mint_wrapped(network='mumbai', amount0ToMint=0.02*10**TOKEN0_DECIMAL_MUMBAI, amount1ToMint=0.0001*10**TOKEN1_DECIMAL_MUMBAI, price_lower=500, price_upper=2000, max_fee_per_gas=MAX_FEE_PER_GAS, max_priority_fee_per_gas=MAX_PRIORITY_FEE_PER_GAS): 
    ...
    mint(network, amount0ToMint, amount1ToMint, price_lower, price_upper, max_fee_per_gas, max_priority_fee_per_gas, address=Address, private_key=privateKey)

if __name__ == '__main__':
    if len(sys.argv) == 5:  # network, amount0ToMint, amount1ToMint, price_lower, price_upper
        mint_wrapped(network=str(sys.argv[1]), amount0ToMint=eval(sys.argv[2]), amount1ToMint=eval(sys.argv[3]), price_lower=eval(sys.argv[4]), price_upper=eval(sys.argv[5]))
    elif len(sys.argv) == 7:  # network, max_fee_per_gas, max_priority_fee_per_gas
        mint_wrapped(network=str(sys.argv[1]), amount0ToMint=eval(sys.argv[2]), amount1ToMint=eval(sys.argv[3]), price_lower=eval(sys.argv[4]), price_upper=eval(sys.argv[5]), max_fee_per_gas=eval(sys.argv[6]), max_priority_fee_per_gas=eval(sys.argv[7]))
    else:   
