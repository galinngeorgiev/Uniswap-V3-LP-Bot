...
def mint(network='mumbai', amount0ToMint=0.02*10**TOKEN0_DECIMAL_MUMBAI, amount1ToMint=0.0001*10**TOKEN1_DECIMAL_MUMBAI,\
    ...
    result = wait_for_transaction_receipt(network, tx_hash, timeout=MINT_TIMEOUT_SEC, polling=RECEIPT_POLLING_SEC, init=init)
    if isinstance(result, dict):
        ...
        logger.error(network + ', mint() failed because wait_for_transaction_receipt() failed')
        return False
