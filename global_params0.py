...
if TEST:
	...
	NUM_LP, RUNTIME_SEC, INIT_LP_POSITION_TO_INIT_PRICE = 1, 72000, -1 #N.B. Running time of the loop (loop runs longer because tx executions take time)
	...
	UNWIND_DIST_TO_BOUND_PER = [(-0.15, -0.15)] #N.B. Unwind j-th tx if its dist-to-...-bound < dist-to-...-bound; greater in abs value UNWIND_DIST_TO_BOUND_PER do not incure OTM loss!
	...
	MIN_SESSION_SWAP_PER, MIN_TX_BATCH_SWAP_PER, SWAP_EPSILON_PER = 100, 1000000, 10 #N.B. execute swaps only if abs(amount_to_swap_token0) > SWAP_EPSILON_PER / 100  * sum(tx_borrow_invested_token1) * price
